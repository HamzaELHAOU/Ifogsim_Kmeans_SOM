...
public class test_topologies {
    ...
    static double SENSOR_TRANSMISSION_TIME = 15;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (Config.ENABLE_DYNAMIC_CLUSTERING) {
                ...
                controller = new ClusteringController("master-controller", fogDevices, sensors, actuators, locator, clusteringLevels);
            } else {
                controller = new ClusteringController("master-controller", fogDevices, sensors,actuators, locator, clusteringLevels);
            }
    ...
    private static Application createApplication(String appId, int userId) {
         ...
         */
        application.addAppModule("clientModule", 20); // adding module Client to the application model
        application.addAppModule("processingModule", 20); // adding module Concentration Calculator to the application model
        application.addAppModule("storageModule", 20); // adding module Connector to the application model
        application.addAppModule("DataFilteringModule", 20);
        application.addAppModule("EventHandlerModule", 20);
        ...
        if (SENSOR_TRANSMISSION_TIME == 5.1)
            application.addAppEdge("M-SENSOR", "clientModule", 2000, 500, "Health-Sensor", Tuple.UP, AppEdge.SENSOR); // adding edge from EEG (sensor) to Client module carrying tuples of type EEG
        else
        	  
       
        application.addAppEdge("M-SENSOR", "clientModule", 3000, 500, "Health-Sensor", Tuple.UP, AppEdge.SENSOR);
        application.addAppEdge("clientModule", "DataFilteringModule", 3500, 500, "RAW_DATA", Tuple.UP, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
    application.addAppEdge("processingModule", "storageModule", 1000, 1000, "storage_DATA", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("DataFilteringModule", "processingModule", 1000, 1000, "filtered-data", Tuple.UP, AppEdge.MODULE);// adding periodic edge (period=1000ms) from Concentration Calculator to Connector module carrying tuples of type PLAYER_GAME_STATE
        application.addAppEdge("processingModule", "EventHandlerModule", 1000, 1000, "PROCESSED_DATA", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("EventHandlerModule", "clientModule", 14, 500, "response", Tuple.DOWN, AppEdge.MODULE);  // adding edge from Concentration Calculator to Client module carrying tuples of type CONCENTRATION
        application.addAppEdge("clientModule", "M-DISPLAY", 1000, 500, "action", Tuple.DOWN, AppEdge.ACTUATOR);  // adding edge from Client module to Display (actuator) carrying tuples of type SELF_STATE_UPDATE
         ...
         */
         application.addTupleMapping("clientModule", "Health-Sensor", "RAW_DATA", new FractionalSelectivity(1.0)); // 0.9 tuples of type _SENSOR are emitted by Client module per incoming tuple of type EEG
        application.addTupleMapping("DataFilteringModule", "RAW_DATA", "filtered-data", new FractionalSelectivity(1.0));
        application.addTupleMapping("processingModule", "filtered-data", "storage_DATA", new FractionalSelectivity(1.0));
        application.addTupleMapping("processingModule", "filtered-data", "PROCESSED_DATA", new FractionalSelectivity(1.0)); // 1.0 tuples of type SELF_STATE_UPDATE are emitted by Client module per incoming tuple of type CONCENTRATION
        application.addTupleMapping("EventHandlerModule", "PROCESSED_DATA", "response", new FractionalSelectivity(1.0)); // 1.0 tuples of type CONCENTRATION are emitted by Concentration Calculator module per incoming tuple of type _SENSOR
        application.addTupleMapping("clientModule", "response", "action", new FractionalSelectivity(1.0)); // 1.0 tuples of type GLOBAL_STATE_UPDATE are emitted by Client module per incoming tuple of type GLOBAL_GAME_STATE
        ...
        final AppLoop loop1 = new AppLoop(new ArrayList<String>() {{
            ...
            add("EventHandlerModule");
            add("DataFilteringModule");
            add("clientModule");
            add("M-DISPLAY");
